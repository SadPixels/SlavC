...
RusLang — прототип компилятора русскоязычного языка для .NET 10. Компилятор
...
Текущий frontend намеренно минимален и обрабатывает программу построчно.
...
Текст без кавычек также допустим:
...
```text
# Это комментарий
печать "Программа запущена"
```

Пустые строки игнорируются. Исходные файлы читаются как UTF-8.

### Режим C#

Для проверки полного pipeline Roslyn и RuntimeHost файл можно начать директивой
`#csharp`. Остальная часть файла будет передана компилятору как C#:
...
Это временный отладочный режим, пока полноценные parser, AST и semantic analysis
RusLang расширяются.
