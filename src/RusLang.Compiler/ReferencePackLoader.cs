...
public static class ReferencePackLoader
{
    ...
    private static IReadOnlyList<MetadataReference> LoadCore()
    {
        ...
        var names = owner.GetManifestResourceNames()
            .Where(static name => name.StartsWith("RusLang.ReferencePack.", StringComparison.Ordinal) &&
        ...
        if (names.Length > 0)
        {
            return names.Select(name =>
            {
                using var stream = owner.GetManifestResourceStream(name)
                    ?? throw new InvalidOperationException($"RUSC2001: Missing reference resource '{name}'.");
        ...
        if (refPath is null)
        {
            throw new InvalidOperationException(
                "RUSC2002: Embedded net10.0 reference pack is unavailable.");
        }
