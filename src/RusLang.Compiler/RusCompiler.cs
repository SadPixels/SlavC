...
public sealed class RusCompiler
{
    public CompilationArtifact Compile(CompilationRequest request, CancellationToken cancellationToken = default)
    {
        ...
        var (generatedSource, frontendDiagnostics) =
            RusSourceEmitter.Emit(sourceText, Path.GetFullPath(request.SourcePath));
        if (frontendDiagnostics.Any(static value => value.Severity == RusDiagnosticSeverity.Error))
        {
    ...
    private static RusDiagnostic TranslateDiagnostic(Diagnostic diagnostic, string fallbackPath)
    {
        var span = diagnostic.Location.GetLineSpan();
        return new RusDiagnostic(
            diagnostic.Id,
            diagnostic.Severity switch
            {
                DiagnosticSeverity.Error => RusDiagnosticSeverity.Error,
                DiagnosticSeverity.Warning => RusDiagnosticSeverity.Warning,
                _ => RusDiagnosticSeverity.Info,
            },
    ...
    private static string SanitizeAssemblyName(string value)
    {
        var sanitized = new string(value.Select(character =>
            char.IsLetterOrDigit(character) || character is '_' or '.'
                ? character
                : '_').ToArray());
        return string.IsNullOrWhiteSpace(sanitized) ? "RusLangProgram" : sanitized;
    }
}
