...
{
    private readonly SlavPackReader reader;
    private readonly Dictionary<string, SlavPackEntry> assemblies;
    private readonly HashSet<string> resolving = new(StringComparer.OrdinalIgnoreCase);
    private readonly bool trace = Environment.GetEnvironmentVariable("SLAVLANG_HOST_TRACE") == "1";

    public ManagedBundleLoadContext(SlavPackReader reader)
        : base("SlavLang.ManagedBundle", isCollectible: false)
    {
        this.reader = reader;
        assemblies = reader.Manifest.Entries
            .Where(static entry =>
                entry.Kind is SlavPackEntryKind.MainAssembly or SlavPackEntryKind.ManagedAssembly)
            .ToDictionary(
    ...
    public Assembly LoadEntryAssembly()
    {
        ...
        var pdbEntry = reader.Manifest.Entries.FirstOrDefault(candidate =>
            candidate.Kind == SlavPackEntryKind.PortablePdb &&
            string.Equals(
    ...
    private void Trace(string message)
    {
        if (trace)
        {
            Console.Error.WriteLine($"[SlavLang host] {message}");
        }
    }
}
